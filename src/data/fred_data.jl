...
function load_fred_data(m::AbstractModel;
    ...
    vint = data_vintage(m)
    dateformat = "yyyymmdd"  # Can I set to the DFM_DATE_FORMAT, only if i include in the abstractdfmmodel type

    # # Have to do this wacky parsing to prepend the century to the data vintage
    # # Only if using the yymmdd format, which is why I changed that format choice
    ...
    if isfile(datafile)
        ...
        # Fred data is organized by end of quarter in TS notation
    ...
    if !isempty(missing_series)
        ...
        # FredSeries are types that hold all information passed from Fred, constructed in the FredData.jl package
        fredseries = Array{FredSeries, 1}(length(missing_series))
        # Initializes the FredData.jl algorithm to look for API key and establish connection
        f = Fred()
        ...
        for (i,s) in enumerate(missing_series)
            ...
            try
                #TODO must eventually change the frequency of the model here.
                #   Hoping that this will output a higher frequency series (i.e. GDP) with NA holes (this is the form I want for sending matrix of Float64 to the Kalman)
                # get_data is from FredData.jl and queries the API with key initialized in earlier step
