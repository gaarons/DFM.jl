...
that we have at least a single datapoint at the begining of the sample

TODO put back the language about containing dates start to end from DSGE codes
...
function isvalid_data(m::AbstractModel, df::DataFrame)
    ...
    if !isempty(coldiff)
        println("Columns of 'df' do not match requested m.observables and m.datatransforms")
        println(coldiff)
    end
    ...
    # TODO check is all nans at the start date... do we estimate a factor back to that start date == need data there
    # # Ensure that we have data at the date_mainsample_start
    # m_series = collect(keys(m.data_transforms))
    # df_series = names(df)
    # coldiff = setdiff(m_series, df_series)
    # valid = valid && isempty(coldiff)
...
# isna across the whole row of find(:date == :date_mainsample_start)
...
suitable for direct use in <estimate> functions
TODO might need to order matrix for frequency if cannot detect in the DFM algo
"""
function df_to_matrix(m::AbstractModel, df::DataFrame)
    ...
    # Leave only data for the next step, :date no longer included
